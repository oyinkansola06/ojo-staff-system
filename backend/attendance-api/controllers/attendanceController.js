...
class AttendanceController {
    ...
    static async getAttendanceStats(date) {
        try {
            ...
            const attendedCount = stats.present_count + stats.late_count + stats.half_day_count;
            stats.attendance_rate = stats.total_staff > 0 
    ...
    static async createManualEntry(req, res) {
        try {
            ...
            const [result] = await pool.execute(`
                ...
                notes = VALUES(notes),
                updated_at = CURRENT_TIMESTAMP
            `, [staff_id, attendance_date, time_in, time_out, status, hoursWorked, notes]);
